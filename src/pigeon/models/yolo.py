...
def yolov7(
    ...
    img_det = np.array(img_det, dtype=np.float32) / 255.0
...
def yolo26(img_det, interpreter_detect, score_threshold, ratio, pad) -> list[Object]:
    input_details = interpreter_detect.get_input_details()
    output_details = interpreter_detect.get_output_details()

    # quantize
    scale_i8, shift_i8 = input_details[0]["quantization"]
    img_det_float = np.array(img_det, dtype=np.float32) / 255.0
    ...
    return [
        Object(
            id=coco80_to_coco91_class(final_classes[i]) - 1,
            score=final_scores[i],
            bbox=BBox(xmin=b[0], ymin=b[1], xmax=b[2], ymax=b[3]),
        )
